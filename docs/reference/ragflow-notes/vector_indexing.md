...
后端,统一封装在 `DocStoreConnection` 抽象类 (`doc_store_base.py`)
...
- **BM25 + 向量要在同一个 `search` 里融合,不是分两次取**。RAGFlow 的
  `ESConnection.search` (`rag/utils/es_conn.py`) 用
  `MatchTextExpr / MatchDenseExpr / FusionExpr` 拼一个 `bool` 查询,
  文本打分 (`query_string`) 和向量打分 (`s.knn`) 在同一轮 ES 查询里
  出分,再按 `vector_similarity_weight=0.5` 加权融合 (`es_conn.py`)。
