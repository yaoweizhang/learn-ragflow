...
| 脚本 | 解决什么 | 留下什么局限 | 何时用 |
|---|---|---|---|
| `substring_match.py` | 字面命中 (query 出现在 chunk 中) | 找不到同义词 ("营收" 找不到 "营业收入"); 无打分 | 教学 / 强字面约束场景 (法条 / 代码标识符) |
| `bag_of_words.py` | 词频向量 + 余弦排序 top-k | 维度爆炸; 丢语序 ("狗咬人" = "人咬狗"); 仍无真语义 | 玩具检索 / 教学 / 小语料 |
| `rag_pipeline.py` | retrieve → build_prompt → call_llm 完整闭环 | 召回错则全错; prompt 极简; 无 rerank | 端到端 demo / 后续章节填空底座 |
