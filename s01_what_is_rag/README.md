...
> **章节定位**：本章是 12 章的入门。3 个脚本递进：朴素子串 → 词袋向量 → 完整 RAG 链路（检索 + Prompt + LLM）。跑完这 3 个脚本，就拿到了 RAG 全链路的最小闭环。
>
> **章节结构**：3 个脚本都独立可跑、自包含。s02-s12 会把本章 3 个脚本里的玩具逐步替换成工业实现。
>
...
```bash
python s01_what_is_rag/code_01_naive_keyword.py        # 01：子串匹配，零依赖
python s01_what_is_rag/code_02_vector_basics.py        # 02：词袋向量，零依赖
python s01_what_is_rag/code_03_augmented_llm.py        # 03：无 key 时只打印 prompt
LLM_API_KEY=sk-xxx python s01_what_is_rag/code_03_augmented_llm.py   # 有 key 时真调 LLM
```

依赖：`python-docx`（读样本 DOCX）；03 额外要 OpenAI 兼容 LLM 的 `LLM_API_KEY`。
...
- **检索质量**：01（子串）→ 02（sparse 向量）→ s04（dense BGE）→ s06（BM25+dense 融合）
- **生成质量**：01（直接返回）→ 03（prompt+LLM）→ s07（rerank 后喂 LLM）→ s08（工业 prompt 模板）
...
### 01 / Q1. 怎么把子串匹配改成 Top-3 候选段？最简单的打分怎么算？
...
### 02 / Q1. 如果两段都包含"披露"两次，词袋向量会怎么算？它分得开"摘要式披露"和"详细披露"吗？
...
### 03 / Q1. 如果 LLM 答了一段不在 `<context>` 里的话（比如"按惯例审计费用通常为 50 万元"），怎么从工程上防住？
