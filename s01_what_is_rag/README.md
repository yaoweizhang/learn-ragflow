...
大语言模型（LLM，Large Language Model，下面简称"大模型"）很会"说话", 但不会"答"。它不知道你公司昨天签的合同, 不知道上季度的财报数字, 不知道你团队昨天改的接口签名 — 不是它笨, 而是**它只见过训练截止日期之前的公开世界**。把这三种典型失败场景拆开看, 都是同一类问题的不同切面:

**第一, 训练截止 (knowledge cutoff, 训练数据的截止时间)**。任何 LLM 都有一个训练数据截止时间, GPT-4 是 2023 年 10 月, Claude 是 2025 年初, 开源模型也类似。问它昨天的股价、最新法规、刚发布的论文摘要, 它要么沉默 ("截至我的训练数据..."), 要么编一段听起来合理但完全错误的内容。这个问题**没有银弹** — 模型的训练数据是固化的, 你没法在每次新数据出现时重训整个模型, 工业级的成本和时间都不允许。即便是号称"在线学习"的方案, 也只能做轻量微调（fine-tune，见 s00 §2.1）, 没法把整份新文档塞进参数。

**第二, 私有数据 (private data, 模型没见过的内部数据)**。你的客户名单、内部 wiki、产品需求文档、合同附件、技术评审记录 — 这些数据从来没出现在任何公开训练集里。即便你用的是 GPT-5 还是 Claude-Opus, 它也读不到你没喂给它的东西。把整份公司文档塞进 prompt? 不现实, 一份 10 万字的 wiki 已经超出大多数模型的上下文窗口; 即便塞得下, 也稀释了模型对关键信息的注意力 (**lost-in-the-middle 现象**: 模型对长 prompt 中间段的注意力远低于首尾)。RAG 用"按需检索 + 拼 prompt" 的方式绕过这个限制: 不一次性塞整份文档, 只检索 top-k（得分最高的 k 个候选）相关段落拼进 prompt。

**第三, 幻觉 (hallucination, 模型编造细节, 见 s00 §1.1)**。在没有外部资料约束的情况下, LLM 会**自信地编造细节** — "按惯例审计费用通常为 50 万元", 这个数字是它从训练语料里"看起来合理"地拼出来的, 不是从任何一份资料里查到的。幻觉是 LLM 最大的可信度杀手: 用户看到流畅、自信、带具体数字的回答, 不会怀疑它在编; 等到出问题时 (法务纠纷、财务误报、医疗误诊), 代价已经付出去。即便加了"如果你不知道请说不知道"的 prompt（提示词,见 s00 §2.1）约束, LLM 在压力下仍可能编造, 因为它的训练目标就是"对所有 query 给出流畅回答", 拒答对模型来说是低概率事件。
...
s01 的任务就是把这三步用 **30-80 行 Python 跑通一遍**, 不依赖任何 embedding（把文本转成实数向量的过程,见 s04）模型或向量库（可按相似度查询的向量数据库,见 s05） — 让"retrieve + augment + generate" 这三个动词绑死在一条线上, 后面 11 章再把每个动词替换成工业实现。如果你想只跑通 RAG 的最小形态, 读完本章即可; 如果想上生产, 继续读 s02-s12。
...
| 脚本 | 解决什么 | 留下什么局限 | 何时用 |
|---|---|---|---|
| `substring_match.py` | 字面命中 (query 出现在 chunk[文档切块,见 s03] 中) | 找不到同义词 ("营收" 找不到 "营业收入"); 无打分 | 教学 / 强字面约束场景 (法条 / 代码标识符) |
| `bag_of_words.py` | 词频向量（每个维度是该词在文本中出现的次数） + 余弦排序 top-k | 维度爆炸; 丢语序 ("狗咬人" = "人咬狗"); 仍无真语义 | 玩具检索 / 教学 / 小语料 |
| `rag_pipeline.py` | retrieve → build_prompt → call_llm 完整闭环 | 召回错则全错; prompt 极简; 无 rerank（重排,见 s07） | 端到端 demo / 后续章节填空底座 |
...
2. 接收交互输入的 query（用户提问,如 `披露`, `外星人`） — `input("问点啥: ").strip()` 去掉首尾空白
3. 把 query 用 `question.split()` 拆成词列表, 段落用 `.lower()` 转小写（让大小写不影响匹配）
4. 遍历段落, 对每段判断"是否有任意 query 词作为子串（连续字符片段）出现在段落中" (`any(w.lower() in p.lower() for w in question.split())`)
5. 第一个命中的段落直接返回; 全 miss 时返回 `I don't know.` — 拒答兜底（没答案就明说"不知道",避免模型自由发挥）, 不让 LLM 编造答案
...
**做一件事**: 把每条 chunk 表征为 2-gram（相邻 2 字组成的词元）词频向量, 用余弦相似度（cosine similarity,两个向量夹角的余弦,夹角越小分越高）排序 top-k 候选, 替代 代码 1 的"第一段即答案"。

**6 步**:
1. 用同款 `python-docx` 加载器读段落 (与 代码 1 共用 `load_paragraphs`)
2. 对每段做 2-gram 切词（tokenize, 把文本切成最小处理单元） (`text[i:i+2] for i in range(len(text)-1)`, 滑动窗口) — 中文每相邻 2 字为 1 个 token（词元,这里是 2-gram 字符片段）, N 字输入产生 N-1 个 token, 保留所有 2 字组合的重叠
3. 全部段落 token 组成全局词表（vocab,所有 unique token → 列索引的字典） `vocab: {token: index}`, 用 `dict.setdefault(tok, len(vocab))` 自动分配索引（只在新 token 出现时分配,已有的复用,索引不会飘移）
4. 每段转成词频向量 `vec = [词频 in vocab]`, 长度 = `len(vocab)`, 用 `Counter(tokenize(text))` 计数
5. query 用同款 2-gram 切, 转同样形状的向量 (query 词表外的 token 维度为 0)
6. 算 query 向量与每段向量的余弦相似度 (`dot(a,b) / (norm(a) * norm(b))`), 排序返回 top-3（分最高的 3 个）
...
6. `call_llm`: 调 OpenAI 兼容 `/chat/completions`（OpenAI 风格的 HTTP API,多数云端 LLM 服务都实现了这套接口） (`urllib.request` 零 SDK 依赖); 无 `LLM_API_KEY` 时只打印 prompt 后停 (教学兜底, 便于无 key 机器验证链路)
...
- **代码 1 找不到同义词** — 字面匹配的固有限制, query `营收` 找不到 chunk 里的 `营业收入`, query `披露` 找不到 chunk 里的 `公开 / 公告`。这是 代码 2 词袋 → s04 BGE 真语义要解决的召回（从候选集合里捞出相关段落）质量问题。
- **代码 2 词袋丢语义 + 丢语序** — 维度爆炸 (2-gram 笛卡尔积), 稀疏向量（大部分维度为 0 的向量,见 s04）无真语义, "狗咬人" = "人咬狗" 同分, "摘要式披露" = "详细披露" 同分。这是 s04 BGE dense 512 维向量要解决的语义召回。
- **代码 3 召回错 + prompt 极简 + 无 rerank（重排,见 s07）** — 召回质量是整条流水线的天花板 (top-3 不相关, LLM 拿到错的资料就编答案); prompt 无引用跟踪会让 LLM 编 `[5]` 但 prompt 里只有 `[1][2][3]`, 这是 s06 hybrid（混合检索,BM25+dense 加权融合,见 s06）+ s07 rerank + s08 工业 prompt 要解决的"召回 + 精排 + 生成" 全链路加固。

s02 **文档加载**: 给 RAG 一份"能读"的资料 — 把 `python-docx` 单文件解析扩成 PDF + DOCX + TXT 多 Parser 调度, 产出统一的 `{text, page, source}` schema（数据结构约定,见 s02）, 让 代码 2 的输入不再依赖单一文件类型, s03 chunking（文本分块,见 s03）才有干净原料。这是把"玩具 RAG"推向"工业 RAG"的第一步 — 没有稳定的文档加载, 后面的 chunking / embedding / retrieval 都建立在沙滩上。
...
要解决: 代码 2 词袋模型把 query 和 chunk 都转成 2-gram 词频向量, 余弦相似度能在部分情况下给出非零分 (比如 `披露` 和 `公开披露` 共享 `披露` 这个 2-gram); 但更彻底的解决是 s04 的 BGE 真语义向量, 把语义相关的词压到向量空间（每个维度代表"语义特征"的实数空间）的近邻位置 — 即便字符完全不重合, "披露" 和 "公告" 的 BGE 向量也会有高余弦相似度。
