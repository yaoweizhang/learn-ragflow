...
术语速览 (本文件首次出现):
- 词袋 (BoW, Bag-of-Words): 文本→向量的最朴素做法,只统计词频,忽略顺序和语法
- 词频向量: 把段落里每个词的出现次数排成向量,词表外记 0
- 余弦相似度 (cosine similarity): 两个向量的夹角余弦,值越大越相似;等价 dot(a,b)/(‖a‖·‖b‖)
- 词表 (vocab): 全部段落 token 集合,作为向量维度索引
- top-k: 排序后取分数最高的前 k 个结果
"""
import math
import re
