...
术语速览 (本文件首次出现):
- RAG (Retrieval-Augmented Generation): 检索增强生成,先从资料里召相关段落再让 LLM 据此回答
- prompt: 给 LLM 的指令文本;此处把召回段落塞进 <context> 标签让模型参考
- LLM: 大语言模型,此处用 OpenAI 兼容 chat completions API
- temperature: 生成温度参数,0 偏确定、1 偏多样;此处 0.2 让回答稳定
- 拒答兜底 ("我不知道"): prompt 里明确告诉模型"资料不足时不要瞎编",避免幻觉
"""
import math
import os
import re
