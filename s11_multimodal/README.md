...
| Unit | 主题 | 它解决什么 |
|---|---|---|
| [01_table_extract](#unit-01--表格抽取-pdfplumber-code_01_table_extractpy) | `pdfplumber.extract_tables()` + 空表过滤 + 行级 chunk 友好输出 | "表格被 `pypdf` 拍扁" → "按行列原样抽" |
| [02_ocr](#unit-02--ocr-pytesseract-code_02_ocrpy) | `pytesseract.image_to_string(..., lang="chi_sim+eng")` + 三类异常优雅跳过 | "扫描件返回空字符串" → "图片里的字转成字符串" |
