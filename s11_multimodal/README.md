...
>
> 注: `pdfplumber`(Python PDF 解析库,基于 pdfminer.six,可读到字符 / 矩形 / 线条等底层对象,常用于表格抽取);`pytesseract`(Python 调用系统 tesseract 二进制的薄封装包,本身不做识别,把图交给 tesseract);`Pillow`(PIL 的现代分支,Python 图像读写库,`Image.open` 打开图);`chi_sim`(tesseract 的简体中文语言包,`lang="chi_sim+eng"` 才会认简体字)。
...
**第三,乱码判定误伤 — 纯文本 PDF 被强制走 OCR,跑 5–10 倍时间**。这是生产里更隐蔽的坑:subset 字体(s02 已介绍)把 CJK 映射成 ASCII(PUA / font-encoding garbling),看起来是 ASCII 字符、其实是有毒文本层。`_is_garbled_text`(乱码判定函数:统计连续不可打印字符或异常拉丁字符的比例,超阈值则视为乱码文本层)阈值过低会误判,**让一份纯文本 PDF 也跑 5–10 倍 OCR 时间**。MVP 不做乱码检测 — 假设 PDF 是干净的,复杂场景切 RAGFlow。
...
> 注: `pypdf`(纯 Python PDF 文本提取库,s02 主力,按行扫文字流,输出"列与列粘连"的纯文本);`extract_tables`(pdfplumber 的启发式画线抽表 API,返回 `list[list[cell]]` 二维结构);`image_to_string`(pytesseract 的核心 API,把图片转成纯字符串)。
...
2. 对每页调 `page.extract_tables()` — **启发式画线算法**(用页面上的矩形 / 线条检测网格,沿网格线划 cell 边界:对**带边框 / 带网格线**的规整表格(白皮书规格表、CSV-like 表)很顶)
...
2. 用 Pillow `Image.open(Path(img_path))` 打开图片;调 `pytesseract.image_to_string(img, lang="chi_sim+eng")` 转交**系统 tesseract 二进制**做识别(`lang="chi_sim+eng"` 同时支持简体中文 + 英文;`TesseractNotFoundError` 是 pytesseract 在系统找不到 tesseract 二进制时抛的专用异常)
...
- **没有版面分析**(**版面分析**:Layout Analysis,把页面切分为标题 / 正文 / 表格 / 图片等不同区域并标注类型;tesseract 不做版面分析,只输出纯文本流) — 输出是平铺字符串,要下游自己用空白 / 标点切段落
- **不识别表格结构** — 表格单元格的字能读出来,但**行列结构丢了**;生产要么走工业 `TableStructureRecognizer`(表格结构识别模型,先检测表格区域、再识别行列边界、最后 per-cell OCR),要么上 PaddleOCR / mineru(国产开源文档解析工具,版面分析 + 表格结构 + OCR 一体)
- **依赖系统二进制** — 服务器部署 / Docker 镜像要单独装 `tesseract-ocr` + `tesseract-ocr-chi-sim`,CI 容易漏
- **中文准确率 80–90%** — 低分辨率扫描、字体倾斜、加粗混排都会掉;生产通常要 ① 放大 2–3 倍再 OCR;② 加 jieba 分词(s06 已介绍) + **编辑距离**(Levenshtein distance,两个字符串通过插入 / 删除 / 替换互相转换的最少操作数,用于 OCR 纠错)纠错;③ 上**视觉 LLM**(能"看图"的 LLM,如 GPT-4V / Qwen-VL,直接读图输出结构化文本)
...
OCR 是在**栅格化的图像**（把矢量 / 文本格式转成像素图,如 PDF 渲染为 PNG）上跑的（这张图可能是 `pdfplumber` 把整页渲染成 300 DPI 的 PNG）。要把图像坐标 `(x_img, y_img)` 变回 PDF 坐标 `(x_pdf, y_pdf)`：
...
> 注: **DPI**(Dots Per Inch,每英寸像素数;PDF 默认 72 DPI,扫描件常用 300 DPI,渲染比 = DPI/72);**bbox**(bounding box,边界框,通常为 `(x0, top, x1, bottom)` 描述字 / 行 / 段的外接矩形)。
...
- 跟 PDF 的文本层做**重叠检测**(**IoU**,Intersection over Union,两个矩形交集面积除以并集面积,值越大越重叠;> 0.7 视为同一段)——如果 OCR 段跟文本层某段重叠，说明文本层有，OCR 是冗余备份；
...
行数（< 2 行大概率是页眉）+ 列数（< 2 列大概率是文本块）+ 单元格内文本长度分布（短文本成片出现可能不是表）+ 边框检测（`page.lines` / `page.rects`(pdfplumber 暴露的页面上所有矢量线和矩形,用于判断是否有表格边框)是否构成闭合矩形）。或者直接用 `LayoutRecognizer`(版面识别器,基于视觉模型把页面切分并标 `type=="table" | "text" | "figure"` 等区域,s02 已介绍)标 `type=="table"` 再信。MVP 走"双重循环 + 至少一行有非空白 cell"启发式,对干净 PDF 够用,对"页眉碰巧排成表格形"会误判。
