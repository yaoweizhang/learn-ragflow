...
两段独立:表格抽取从 PDF 拿 `list[{"page", "rows"}]`、OCR 从图片拿字符串,各自有自己的入口和输出。**下游如何接入**:表格抽取输出的 `rows` 按行切后喂 s03 chunking(s02 主线复用);OCR 输出的字符串进 s03 chunking(走"行/段"启发式)。两者都不进 s11 自身的检索——s11 只到"输出可被下游消费"为止,不做下游。

两者通过相同的"纯文本数据流"汇入 s04-s08 主线,所以"多模态 RAG 不是新管线,是 s02 主线的预处理插件"——下游不需要分情况处理。
