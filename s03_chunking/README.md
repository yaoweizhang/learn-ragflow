...
## 二、最小可跑分块：500 字符 cap + 句界切：[c01_basic_chunk.py](c01_basic_chunk.py)
...
- 三类隐形失败（表格被切碎 / 父子块缺失 / 跨段引用断裂）：见 c02 `### 概念` 的完整对照表，下面 `c02` 的 `### 跑一遍` 把每条都各跑一遍。
...
## 三、chunker 在真实样本上的三类失败：[c02_chunk_failures.py](c02_chunk_failures.py)
...
| 函数 | 文件 | 输入 | 输出 | 一句话解释 |
|---|---|---|---|---|
| `split_long_paragraph(text, max_chars)` | `c01_basic_chunk.py` | `str`, 整数 cap | `list[str]` | 超长段落按 `[.。!?！？]` 切，再贪心装桶；单句超 cap 按字符硬切兜底 |
| `chunk_by_paragraph(docs, max_chars=500)` | `c01_basic_chunk.py` | `list[dict]`, 整数 cap | `list[dict]` | 短段整段保留；长段调 `split_long_paragraph`；每块带 `chunk_id` |
| `main()` (01) | `c01_basic_chunk.py` | — | 打印块数 + 最大长度 + 前 3 个 chunk | 01 演示入口 |
| `demo_table_split()` | `c02_chunk_failures.py` | — | 打印规格表 hard-cut 前后 | 失败 a：表格被切到 mid-row |
| `demo_parent_child()` | `c02_chunk_failures.py` | — | 打印 header-only chunk 与数据本体 | 失败 b：父子块缺失 |
| `demo_cross_ref()` | `c02_chunk_failures.py` | — | 打印短 chunk 列表 + DOCX 原文 | 失败 c：跨段引用断裂 |
| `main()` (02) | `c02_chunk_failures.py` | — | 调用上面 3 个 demo + 引出工业解法 | 02 演示入口 |
...
把 `c01` 跑在仓库自带的 `samples/` 上，得到的真实片段长这样：
...
- **要先看清错误再选工具** → 用本章 `c02` 的真实样本把损失量化，再决定要不要换。
...
2. 在 `c01_basic_chunk.py` 的 `main()` 里按 `CHUNK_STRATEGY` env 选 chunker（默认 `paragraph`），不要在 `chunk_by_paragraph` 里写 `if strategy == "token": ...` 之类分发——它只懂句界正则；
