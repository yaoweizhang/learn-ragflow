...
> **代码文件**: c01_basic_chunk.py · c02_chunk_failures.py
...
**第二，父子块缺失**。"第四节 分季度财务数据" 这种 10 字符的节标题单独成 chunk，用户问 "Q3 营收" 时检索命中的是标题，数字本身在 DOCX tables 里 — 而 s02 的 `load_docx` 已经把 tables 丢了 (见 s02 `c02_failure_modes` 的量化: 27 段 paragraphs + 3 张表 = 572 字符蒸发)。即便 s02 loader 不丢表，chunker 默认扁平输出也没有 parent-child 关系，检索命中"第四节"这个标题时拿不到季度数字。
...
| 脚本 | 解决什么 | 留下什么局限 | 何时用 |
|---|---|---|---|
| `c01_basic_chunk.py` | 500 字符 cap + 句界正则切 → `{text, chunk_id, page, source}` 四键 schema | 表格被切碎; 节标题单独成 chunk; 指代词断主语 | toy / 教学 / 文本型 PDF + 简单 DOCX |
| `c02_chunk_failures.py` | 量化 01 在真实样本上的三类损失 (规格表 hard-cut / header-only / cross-ref) | **不修** — 只量化; 表格/父子树/层级合并留给 s11 | 教学动机; 决策"要不要跳 s11" |
...
## 代码 1: 最小可跑分块 ([c01_basic_chunk.py](c01_basic_chunk.py))
...
## 代码 2: 真实样本上的三类失败 ([c02_chunk_failures.py](c02_chunk_failures.py))
...
**4 步**:
1. **复用 c01 的函数** — `importlib.util.spec_from_file_location` 加载 `c01_basic_chunk.py`, 复用 `load_pdf` / `load_docx` / `chunk_by_paragraph` / `split_long_paragraph` 不重写
