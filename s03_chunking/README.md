...
1. **token-aware 切分**——把字符 cap 换成 tiktoken 算的 token cap（典型 128 token）。这是最低成本的升级，能解决 "BGE max_seq_len=512 被超限" 问题。
2. **版面感知**——按 XGBoost 识别的 `layout_type` 切：表格块单独保留（不分切），节标题块带 parent 上下文，列表项成组。RAGFlow `pdf_parser.py` 走这条路。
3. **父子块结构**——把"段落级 parent + 句界 child"压成两层结构；检索用 child（小向量、命中准），生成用 parent（上下文全、不丢语义）。`LangChain ParentDocumentRetriever` / `LlamaIndex HierarchicalNodeParser` / `RAGFlow hierarchical_merge` 都走这条路。
4. **媒体回填**——切完后对每个 chunk 调 `attach_media_context`，把附近图片 / 表格的引用追加到 chunk 文本末尾——保证多模态 LLM 能"看图"。
