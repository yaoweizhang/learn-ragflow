...
**实操建议**: 跑 代码 2 看到表格字符数大 / 节标题 chunks 多 / 指代词独立 → 直接跳 s11 看 RAGFlow 的 `hierarchical_merge` + `_concat_downward` + `naive_merge` + `attach_media_context` 4 层流水线怎么集成; 不需要也不应该在 代码 1 上贴膏药 (版面分析 + XGBoost 模型下载成本不该压在入门骨架里)。
