...
运行: python s03_chunking/chunk_failures.py
需要: 同 unit 01

术语速览 (本文件首次出现):
- 父子块 (parent-child chunk): 大块为父含完整语境,小块为子指向父;召回子时回填父
- 跨段引用断裂: "如下"/"见上表"等指代词单独成 chunk,失去上下文
- mid-row 硬切: 在表格行中间按字符数切,把一行切成两半破坏列对齐
- 召回 / 命中: 用户 query 检索时命中的 chunk 列表
- attach_media_context: 给媒体(表格/图片)回填前后段落作为上下文
"""
...
_UNIT01_PATH = Path(__file__).resolve().parents[0] / "basic_chunk.py"
