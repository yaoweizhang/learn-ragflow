...
术语速览 (本文件首次出现):
- 1 跳查询 (1-hop query): 从某个节点出发,只看它直接相连的邻居
- 邻接表 (adjacency list): dict[head] → set[(rel, tail)],本教程图的主要数据结构
- 离线查询: 不调 LLM / 不连数据库,直接读 JSONL 跑查询
- 字母序排序: query_graph 把 1 跳邻居按 (rel, tail) 排序,便于对照输出
"""
...
def main() -> None:
    ...
    if not graph:
        ...
        print("请先跑: python s10_graphrag/extract.py")
        return
