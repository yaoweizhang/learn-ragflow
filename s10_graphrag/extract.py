...
运行: python s10_graphrag/extract.py
需要: 跑通 s02 + s03 + .env 里有 LLM_API_KEY

术语速览 (本文件首次出现):
- GraphRAG: 用知识图谱增强 RAG 召回,解决向量检索漏实体关系的问题
- 三元组 (triple): (head, rel, tail) 表示"头实体—关系—尾实体",如 (北京, 位于, 中国)
- 实体 (entity) / 关系 (relation): 三元组的两个组成要素,实体是节点、关系是边
- JSONL (JSON Lines): 每行一个 JSON 对象的文本格式,适合流式写/读
- response_format=json_object: 让 LLM 直接输出合法 JSON 而不是带 markdown 围栏
- build_graph: 合并多 chunk 三元组成 dict[head] → set[(rel, tail)] 邻接表
"""
import json
import os
import re
