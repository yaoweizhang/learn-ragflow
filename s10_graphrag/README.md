...
| Unit | 主题 | 它解决什么 |
|---|---|---|
| [01_extract](./units/01_extract/README.md) | LLM 抽 (head, rel, tail) 三元组 + JSON 容错解析 + JSONL 持久化 | "LLM 能不能稳定吐 JSON 数组" |
| [02_query](./units/02_query/README.md) | 1 跳图查询（`dict.get` + sorted） | "不调 LLM 的纯内存图查询长什么样" |
