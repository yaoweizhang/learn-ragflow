...
### 2.1 代码干了什么：手写 BM25 召回器（tokenize + 类 + 打分 + topk）
...
### 2.2 跑一遍：单条命令出 34 chunks BM25 top-5
...
### 2.3 实测输出：BM25 top-5 字面命中跨 PDF / DOCX 来源
...
### 2.4 局限与下一步：没语义、中文分词 naive、无字段加权、无倒排、无 query expansion
...
### 3.1 代码干了什么：可注入的 hybrid_topk + α 加权 + 各自归一
...
### 3.3 实测输出：final ≈ α·vec + (1−α)·bm25_norm 的分项证据
...
### 3.4 局限与下一步：per-query α 缺失、无第三层信号、无精排、无分页
