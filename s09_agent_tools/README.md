...
| `_llm(messages)` | `code_01_tool_call.py` | `list[dict]` | `str` | OpenAI 兼容 LLM 调用,剥 `<think>...</think>` 推理块;`@lru_cache` 模型缓存模式同 s08 code_01 |
...
| `main()` (code_01) | `code_01_tool_call.py` | — | 打印 trace | code_01 演示入口,默认 query "R3630 G5 的内存插槽数量" |
| `main()` (code_02) | `code_02_react_loop.py` | — | 打印 trace | code_02 演示入口,默认同上 |
