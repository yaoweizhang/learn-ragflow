...
| Unit | 主题 | 它解决什么 |
|---|---|---|
| [01_tool_call](#unit-01--工具调用单步-code_01_tool_callpy) | 单步工具调用(`TOOLS_DESC` prompt + `_llm` + `_retrieve` + `single_shot`) | "LLM 能不能自己挑 retrieve / finish" |
| [02_react_loop](#unit-02--react-循环-code_02_react_looppy) | ReAct 循环(Thought/Action/Observation + JSON 失败反馈 + `max_steps` 兜底) | "LLM 怎么多步推理 + 怎么不死循环" |
...
| 函数 | 文件 | 输入 | 输出 | 一句话解释 |
|---|---|---|---|---|
| `TOOLS_DESC` | `code_01_tool_call.py` | — | `str` | system prompt 里的"工具描述 + 回答格式"常量(2 工具 + Thought/Action/ActionInput 三行式) |
| `_llm(messages)` | `code_01_tool_call.py` | `list[dict]` | `str` | OpenAI 兼容 LLM 调用,剥 `<think>...</think>` 推理块;`@lru_cache` 模型缓存模式同 s08 unit 01 |
| `_retrieve(query)` | `code_01_tool_call.py` | `str` | `str` | embed → hybrid_search → rerank 拼成可读字符串(self-contained,内联 s05-s07 管线) |
| `single_shot(question)` | `code_01_tool_call.py` | `str` | `dict{text, action, payload, observation}` | 单步工具调用:调 1 次 LLM,解析 Action/ActionInput,执行工具,返回 trace |
| `run_agent(question, max_steps)` | `code_02_react_loop.py` | `(str, int)` | `dict{answer, trace}` | ReAct 主循环:维护 messages,解析 action,路由工具,`max_steps` 兜底,JSON 失败反馈 |
| `main()` (unit 01) | `code_01_tool_call.py` | — | 打印 trace | unit 01 演示入口,默认 query "R3630 G5 的内存插槽数量" |
| `main()` (unit 02) | `code_02_react_loop.py` | — | 打印 trace | unit 02 演示入口,默认同上 |
