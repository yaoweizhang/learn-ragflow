...
| 函数 | 文件 | 输入 | 输出 | 一句话解释 |
|---|---|---|---|---|
| `_llm(messages)` | `code_01_tool_call.py` | `list[dict]` (role/content) | `str` (LLM 输出) | OpenAI 兼容 `/chat/completions`;`temperature=0`;无 key 时返回 `[skipped: ...]` |
| `_embed_model()` / `_embed(texts)` | `code_01_tool_call.py` | `list[str]` | `list[list[float]]` | 复制 s04 BGE-small-zh-v1.5 |
| `_cosine(a, b)` | `code_01_tool_call.py` | `(list[float], list[float])` | `float` | 内积(cosine ≡ inner_product 已 L2 归一化) |
| `_hybrid_topk(docs, query, query_vec, dense_score_fn, k, alpha)` | `code_01_tool_call.py` | `(list, str, list, callable, int, float)` | `list[dict]` | 复制 s06 fusion 公式:`α * vec + (1-α) * bm25_norm` |
| `_reranker()` / `rerank(query, hits, top_k=3)` | `code_01_tool_call.py` | — / `(str, list, int)` | `FlagReranker` / `list[dict]` | 复制 s07 cross-encoder rerank |
| `_retrieve(query)` | `code_01_tool_call.py` | `str` | `str` | s04 + s06 + s07 串联的检索函数(返回格式化文本) |
| `single_shot(question)` | `code_01_tool_call.py` | `str` | `dict{action, observation, final}` | 01 单步:LLM 一次输出 `Thought + Action + ActionInput` → 解析 → 跑工具 → 输出 |
| `main()` (01) | `code_01_tool_call.py` | — | 打印 trace | 01 演示入口;默认 query `"内存"` |
| `run_agent(question, max_steps=5)` | `code_02_react_loop.py` | `(str, int)` | `dict{trace, answer}` | 02 多步 ReAct:循环 `_llm → parse → execute → observation`,`max_steps` 兜底 |
| `main()` (02) | `code_02_react_loop.py` | — | 打印多步 trace | 02 演示入口;同样走 graceful-skip(无 key 时只打印 trace 形状) |
