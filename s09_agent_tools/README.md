...
>
> **章节结构**:本章用 2 个 unit 走完"从单步工具调用到多步 ReAct 循环"——**unit 01** 演示 LLM 能自己挑工具(1 轮),**unit 02** 跑完完整 ReAct 循环(`max_steps=5` 兜底 + JSON 失败反馈)。
> **scope 注意**:本章实现是**手写 ReAct 循环**——system prompt 里塞工具描述 + `Thought/Action/ActionInput` 三行式 prompt,正则抠 action,自己路由工具;不是 LangChain AgentExecutor 那种把 `tool_calls` 结构化字段当一等公民的工业实现。RAGFlow 走的是后者,见 §四。
...
| Unit | 标题 | 入口 |
| --- | --- | --- |
| 01 | 工具调用(单步):把 `retrieve` / `finish` 写进 system prompt,让 LLM 自己挑工具 | [`units/01_tool_call/code.py`](units/01_tool_call/code.py) |
...
```bash
python s09_agent_tools/units/01_tool_call/code.py    # 单步工具调用(1 轮 LLM)
python s09_agent_tools/units/02_react_loop/code.py   # ReAct 循环(最多 5 轮)
...
### 1.3 ReAct 循环的"三行式"格式

每轮 LLM 输出长这样(示例,见 unit 02 的 `run_agent` 主循环):
...
- JSON 解析失败 → 把"原文"当 `Observation` 反馈回去,让 LLM 下一轮自己修正(单元 02 的硬约束)
...
- **unit 01**——跑通最小骨架(`TOOLS_DESC` + `_llm` + `_retrieve` + `single_shot(question)`),演示"1 轮 LLM 就能自己选工具"。把工具调用和 ReAct 循环拆成 2 个 unit 是为了让"LLM 真的能挑工具"和"LLM 怎么多轮推理"分两段讲——单步看到决策能力,多步看到循环控制。
- **unit 02**——在 unit 01 之上加 `run_agent(question, max_steps=5)` 主循环,演示完整 ReAct。复用 unit 01 的所有底座(importlib 加载,代码零重复),新增的 30 行只关心"循环控制 + 终止条件 + JSON 失败反馈"。
...
无 key / 离线环境跑 unit 01 / unit 02:

```bash
unset LLM_API_KEY && python s09_agent_tools/units/01_tool_call/code.py
# 走 graceful-skip:打印 trace 形状 + 假设的 LLM 输出,不调检索管线
```

### 3.3 核心函数一览

s09 的代码拆得很细,每个函数都对应一种"agent 决策 / 工具路由"的角色:

| 函数 | 文件 | 输入 | 输出 | 一句话解释 |
|---|---|---|---|---|
| `TOOLS_DESC` | `units/01_tool_call/code.py` | — | `str` | system prompt 里的"工具描述 + 回答格式"常量(2 工具 + Thought/Action/ActionInput 三行式) |
| `_llm(messages)` | `units/01_tool_call/code.py` | `list[dict]` | `str` | OpenAI 兼容 LLM 调用,剥 `<think>...</think>` 推理块;`@lru_cache` 模型缓存模式同 s08 unit 01 |
| `_retrieve(query)` | `units/01_tool_call/code.py` | `str` | `str` | embed → hybrid_search → rerank 拼成可读字符串(self-contained,内联 s05-s07 管线) |
| `single_shot(question)` | `units/01_tool_call/code.py` | `str` | `dict{text, action, payload, observation}` | 单步工具调用:调 1 次 LLM,解析 Action/ActionInput,执行工具,返回 trace |
| `run_agent(question, max_steps)` | `units/02_react_loop/code.py` | `(str, int)` | `dict{answer, trace}` | ReAct 主循环:维护 messages,解析 action,路由工具,`max_steps` 兜底,JSON 失败反馈 |
| `main()` (unit 01) | `units/01_tool_call/code.py` | — | 打印 trace | unit 01 演示入口,默认 query "R3630 G5 的内存插槽数量" |
| `main()` (unit 02) | `units/02_react_loop/code.py` | — | 打印 trace | unit 02 演示入口,默认同上 |
...
把 unit 02 跑在仓库自带的 `samples/` 上,`run_agent` 返回的 trace 长这样(实测,`MiniMax-M3 over minimaxi.com`):
...
- `LLM 一路 retrieve 不 finish`: 撞 `max_steps=5` 兜底,见 `thinking_answers.md` 思考题 1 的 3 种解法(`max_steps` + prompt 软约束 + 重复 action 检测)。
- `LLM 输出 `Action: retrieve` 但漏 ActionInput`: unit 02 会把"原文"当 Observation 反馈回去让模型下一轮自愈;若连续失败,把 `TOOLS_DESC` 里的格式说明加粗(`**严格按以下格式**`)提升遵从度。
...
2. **ReAct 和 function-calling 的本质区别是什么?**  
   答:ReAct **每轮让模型先输出 Thought(思考过程)再选 Action**,function-calling **直接返回结构化 `tool_calls` 字段、不暴露思考过程**。ReAct 的优势是"可观测性 +300%"(trace 里能看到模型为什么选这个工具),代价是 token +20-30% + 解析脆弱(模型可能吐错格式);function-calling 的优势是"API 帮你解析、鲁棒性归零",代价是"看不到 Thought,production 调试更难"、依赖特定 provider。**MVP 选 ReAct** 是为了把"决策过程"暴露在 README 里;**生产可切 function-calling** 换鲁棒性。详见 [`thinking_answers.md`](./thinking_answers.md)。

3. **工具一多 LLM 选错怎么办?**  
   答:三招:① **按"用户意图"分组路由**——先分类器决定走哪组工具(检索组 / 计算组 / 查询组),不把所有工具一次塞给 LLM;② **工具描述写得"互斥"**——不同工具描述之间不重叠,别让模型在"A 也能做、B 也能做"之间纠结;③ **拆多层 agent**——supervisor 决定调哪个 sub-agent,sub-agent 只看到自己工具集,RAGFlow 用 `Agent` 组件嵌套实现 "agent-as-tool"。详见 [`thinking_answers.md`](./thinking_answers.md)。
