...
> 本章节围绕 *单 backend + HNSW + 元数据过滤* 这一层给出概念 / 问题 / MVP / 工业对照的完整弧线 —— 不引入 Pinecone / Milvus 全表对比（那些是综述性质、留到延伸阅读），也不展开 IVF / LSH / PQ 等其他 ANN 算法（本教程只跑 HNSW）。
...
| Unit | 主题 | 它解决什么 |
|---|---|---|
| unit 01 — `code_01_chroma_build.py` | Chroma 持久化索引构建 (PersistentClient + cosine) | "embedding 怎么从内存落到磁盘，绑上 source/page" |
| unit 02 — `code_02_chroma_query.py` | Chroma 检索 (cosine distance → similarity top-k) | "按 query 向量找 top-k，把分数翻回人类能理解的相似度" |

### 3.2 跑起来

```bash
python s05_vector_index/code_01_chroma_build.py          # 建索引 (~数秒，首次含 BGE 下载)
python s05_vector_index/code_02_chroma_query.py           # 提示 "问: " 后输入中文 query
```
...
| 函数 | 文件 | 输入 | 输出 | 一句话解释 |
|---|---|---|---|---|
| `get_chunks_and_vectors()` | `code_01_chroma_build.py` | — | `(chunks, vectors)` | 加载 samples → 切块 → embed（本地 BGE），返回 `(list[dict], list[list[float]])` |
| `build_index(chunks, vectors)` | `code_01_chroma_build.py` | `(chunks, vectors)` | `chromadb.Collection` | `rmtree` + `PersistentClient` + `create_collection(metadata={"hnsw:space": "cosine"})` + `col.add(...)` |
| `main()` (unit 01) | `code_01_chroma_build.py` | — | 打印索引统计 | 演示入口；加载 → 切 → embed → build → 打印 `indexed N chunks into _chroma/` |
| `_open_collection()` | `code_02_chroma_query.py` | — | `Collection / None` | `PersistentClient.get_collection("docs")`；不存在返 `None` 让 `main()` 提示"先跑 unit 01" |
| `search(col, query_vec, k=5)` | `code_02_chroma_query.py` | `(Collection, list[float], int)` | `list[{text, source, page, score}]` | `col.query(...)` 拿 `documents / metadatas / distances`，`1 - distance` 翻成 `[0, 1]` 相似度 |
| `main()` (unit 02) | `code_02_chroma_query.py` | — | 打印 top-3 hits | 演示入口；`input("问: ...")` → embed → `search(k=3)` → 打印 `[source#page] score=... \| snippet` |
...
## unit 01 — Chroma 持久化索引构建 (PersistentClient + cosine)  (`code_01_chroma_build.py`)
...
生产代码直接弃用 Chroma，选了 `Elasticsearch` 或 `Infinity` 这两个**"BM25 + 向量二合一"**的引擎：原生支持 `dense_vector` 字段、跨节点分片副本、可水平扩到亿级，**多租户硬隔离**靠 `ragflow_<tenant_id>_<kb_id>` 索引命名规则。Chroma 在生产代码眼里只是"开发态玩具"，详细对照见 [`docs/reference/ragflow-notes/vector_indexing.md`](../../docs/reference/ragflow-notes/vector_indexing.md)，里面 ES `create_idx` 的 `mapping.json` 模板把 `settings + mappings`（分片数、副本数、`dense_vector` 字段类型、HNSW 参数）一份文件管全集群。
...
## unit 02 — Chroma 检索：cosine distance → similarity top-k  (`code_02_chroma_query.py`)
...
生产代码在这一层把全文召回（BM25）和向量召回（dense）**用同一个 DB query 一起发**，在 ES / Infinity 侧用 `FusionExpr("weighted_sum", topk, {"weights": "0.05,0.95"})` 融合，再按 API 入参 `vector_similarity_weight` 互补地算全文权重（`search.py:582-637`）；最后 `rerank_with_knn` 用 `tkweight=0.3 / vtweight=0.7` 再做一次线性加权叠 `rank_feature`。**本单元就是生产链路里"dense-only + DB 内部 fusion 前"那个最朴素的切片**——只走了 `MatchDenseExpr` 这一路，没 BM25 兜底也没 PageRank / tag boost。粗召回偏向量（`0.05, 0.95`），精排也偏向量（`0.3, 0.7`），但全文信号从不缺席。
...
提示：cosine = `1 - cos_sim`，`cos_sim = (a·b) / (||a||·||b||)`；向量归一化后 `||a|| = ||b|| = 1`，cosine 距离只剩 `1 - a·b`，跟内积（点积）等价。未归一化时短文本向量天然小、长文本向量天然大，L2 距离会被向量长度主导而不是语义主导——生产代码的 `_rank_feature_scores` 也是同样的"先归一再加权"逻辑（`sqrt(sum(s*s))` 归一）。

## 思考题答案

### Q: 元数据过滤 `source = server_whitepaper.pdf` 怎么写？
