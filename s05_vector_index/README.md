...
>
> **章节结构**：2 个代码文件。code_01 用 Chroma `PersistentClient` 构建 HNSW cosine 索引；code_02 演示 metadata 过滤 + cosine distance → similarity top-k 查询。
>
> **scope 注意**：本章节围绕 *单 backend + HNSW + 元数据过滤* 这一层给出概念 / 问题 / MVP / 工业对照的完整弧线——不引入 Pinecone / Milvus 全表对比（那些是综述性质、留到延伸阅读），也不展开 IVF / LSH / PQ 等其他 ANN 算法（本教程只跑 HNSW）。
...
| 序号 | 标题 | 入口 |
| --- | --- | --- |
| 01 | Chroma 持久化索引构建 (PersistentClient + cosine) | [`code_01_chroma_build.py`](code_01_chroma_build.py) |
| 02 | Chroma 检索：cosine distance → similarity top-k | [`code_02_chroma_query.py`](code_02_chroma_query.py) |

跑法：

```bash
python s05_vector_index/code_01_chroma_build.py    # 把 s04 的向量写进 Chroma PersistentClient
python s05_vector_index/code_02_chroma_query.py    # cosine 检索 + metadata 过滤
```

依赖：`chromadb`（已在 `requirements.txt`）。

样本输入：s04 embedding 输出（4 chunks × 512 维）+ 元数据（`source` / `page`）。
...
### 2.1 Chroma 持久化索引构建 (PersistentClient + cosine)
...
### 2.2 Chroma 检索：cosine distance → similarity top-k
...
Code_02 必须**先**跑过 code_01；否则 `_chroma/` 不存在，`main()` 会打印提示并 `sys.exit(1)`。
...
| `main()` (code_01) | `code_01_chroma_build.py` | — | 打印索引统计 | 演示入口；加载 → 切 → embed → build → 打印 `indexed N chunks into _chroma/` |
| `_open_collection()` | `code_02_chroma_query.py` | — | `Collection / None` | `PersistentClient.get_collection("docs")`；不存在返 `None` 让 `main()` 提示"先跑 code_01" |
...
| `main()` (code_02) | `code_02_chroma_query.py` | — | 打印 top-3 hits | 演示入口；`input("问: ...")` → embed → `search(k=3)` → 打印 `[source#page] score=... \| snippet` |
...
3. 给代码文件 README 加一段"它跟 Chroma 比，赢在哪 / 输在哪"的对照（参考 §四的工具速览）。
