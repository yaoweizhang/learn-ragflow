...
### 2.1 chunk + 向量 + metadata 一起写进 Chroma
...
### 2.4 重建成本、单机瓶颈、维度硬编码、metadata 太薄
...
### 3.1 复用 c01 collection + 同空间 embed + cosine top-k
...
### 3.3 34 chunks × 512 维 + 模型下载与 top-3 解读
...
### 3.4 单进程锁、无 score 阈值、dense-only、没分页
