...
术语速览 (本文件首次出现):
- Chroma: 轻量级向量数据库,内置 HNSW 索引,Python API 简洁
- PersistentClient: Chroma 的持久化客户端,把索引写到本地目录而不是内存
- HNSW (Hierarchical Navigable Small World): 图结构的 ANN 索引算法,查询快但建索引慢
- cosine distance: 余弦距离 = 1 - cosine_similarity,值越小越相似
- hnsw:space: Chroma collection 的元数据,指定距离度量(cosine / l2 / ip)
- metadata (Chroma): 跟向量一起存的字典字段,支持 where 过滤检索
"""
import os
import re
import shutil
