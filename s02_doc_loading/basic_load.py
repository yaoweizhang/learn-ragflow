...
术语速览 (本文件首次出现):
- pypdf: 纯 Python PDF 文本提取库,按行扫文字流
- python-docx: DOCX 解析库,把 Word 段落读成 p.text 字符串
- 统一 schema `list[{text, page, source}]`: 每条记录一段文字 + 页码 + 来源文件名
- 文本层 (text layer): PDF 内嵌的可复制文字流;扫描件 / 图片型 PDF 没有文本层
"""
