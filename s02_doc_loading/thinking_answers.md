...
判定出来后，**对扫描件不要直接 `skip`，而是标记成 `needs_ocr=True`**，把这一页送进 OCR 流水线（RAGFlow 的 `VisionParser` 就是这么做的 —— 见 [`../ragflow_notes/deepdoc_pdf_parsing.md`](../ragflow_notes/deepdoc_pdf_parsing.md)）。
