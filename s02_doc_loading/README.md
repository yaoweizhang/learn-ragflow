...
> **代码文件**: c01_basic_load.py · c02_failure_modes.py
...
| 脚本 | 解决什么 | 留下什么局限 | 何时用 |
|---|---|---|---|
| `c01_basic_load.py` | PDF + DOCX → 统一 `{text, page, source}` schema | DOCX 表格被吞; PDF 多栏错位; 扫描件返回空串 | toy / 教学 / 文本型 PDF + 简单 DOCX |
| `c02_failure_modes.py` | 量化 01 在真实样本上的损失 (PDF 错位 + DOCX 丢 572 字符) | **不修** — 只量化; 表格/扫描件修复留给 s11 | 教学动机; 决策"要不要跳 s11" |

两脚本的关系是一条**教学主干**: 代码 1 把"PDF / DOCX → schema"做出来,暴露"toy 上 OK 真实样本失败"的局限 — 多栏错位让 embedding 拿到乱序文本, DOCX 丢表让关键信息蒸发; 代码 2 把代码 1 喂给真实样本量化损失 (PDF 4 段长度 861/562/413/744 + DOCX 27 段但丢了 572 字符 / 3 张表),暴露"01 在这种语料上不够用"的结论。**s02 看清坑,后续章节填坑 — s03 chunking 缓解 PDF 多栏错位 (切片粒度重排), s11 多模态补全表格 / OCR**。

---

## 代码 1: 最小可跑加载 ([c01_basic_load.py](c01_basic_load.py))
...
## 代码 2: 真实样本上的失败模式 ([c02_failure_modes.py](c02_failure_modes.py))
...
**4 步**:
1. 用 `importlib.util.spec_from_file_location` 加载 `c01_basic_load.py`(目录以数字开头,普通 `import` 报 SyntaxError) — 复用 `load_pdf` / `load_docx` 不重写
...
**实操建议**: 跑 02 看到扫描件率高 / 表格字符数大 → 直接跳 s11 看 RAGFlow 怎么集成;不需要也不应该在 01 上贴膏药(OCR 50MB~1GB+ 的模型下载成本,不该压在入门骨架里)。
