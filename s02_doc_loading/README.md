...
**第一，编码与乱码**。DOCX 在 Windows GBK（国标扩展编码,Windows 中文控制台默认编码）控制台打印时容易报 `UnicodeEncodeError: 'gbk' codec can't encode character '\xa0'`；老 PDF 用非 UTF-8 字符映射（如 GBK / Big5 港台繁体编码）直接出乱码；CJK（中、日、韩三国文字合称）字体的 PDF 还可能因字体子集化（嵌入字体时只保留文档出现过的字形,导致复制/重排时缺字）丢失字符。loader 本身没问题，输出在控制台层就崩了 — 控制台编码是部署层的隐式约束。

**第二，扫描件读不出文字**。`pypdf` 对纯图像页（扫描件、手机拍的合同）`extract_text()` 返回 `""`，整页被静默丢弃。用户以为"我传了 10 页 PDF，loader 应该给我 10 段文本"，实际拿到 0 段；后面 embedding / retrieval 在空文本上直接抛异常或返回空向量。扫描件 detection + **OCR**（Optical Character Recognition,光学字符识别——用图像识别模型把图片里的文字"读"出来）兜底是 s11 主题,本章只识别问题不提供方案。
...
**第四，表格 / 图片丢失或拍扁**。DOCX 的表格存在 `Document.tables` 而非 `Document.paragraphs`，按段读会把整张表吞掉；PDF 的表格在 `extract_text()` 里被拍成没对齐的碎片 (`列1 列2` 一行流，没有 row boundary 行边界)；图片 OCR 文本完全消失。对财报 / 合同 / 规格表这类强结构化文档，表格丢失 = 关键信息全丢 — 不是"丢 30% 召回率"，是"整份文档的核心信息蒸发"。
...
**第一，解析 (Parse)**——按文件格式调不同的库（PDF 走 `pypdf` / `PyMuPDF`，DOCX 走 `python-docx`），从字节流里把文字、表格、图片位置抠出来。**第二，抽取元数据 (Extract)**——元数据（metadata，文档的附属信息：页码、章节标题、作者、创建时间等）对**溯源**（用户问"出处"时能反查）和**过滤**（比如去掉页眉页脚）至关重要。**第三，对齐 schema (Normalize)**——把不同格式的输出整理成同一种数据结构，下游切块器不需要为每种格式重写。
...
4. 打印 "`basic_load.py` 的 load_docx 只读 paragraphs, 丢失 N 字符 (M 张表)" 量化结论 + 指向 RAGFlow 的 `deepdoc/parser/`（RAGFlow 内置的深度文档解析模块）工业解法
...
def main() -> None:
    ...
    print("\n→ RAGFlow 的解法: deepdoc/parser/pdf_parser.py 用 XGBoost（梯度提升树模型,RAGFlow 用来给版面区域分类:标题/正文/表格/图）版面分析;")
...
- **多栏错位未修** — PDF 抽出来的字符串对 RAG 仍然是"被切碎又乱拼"的输入;切片粒度的重排留给 s03 chunking 在 token-cap 级小块上缓解,**版面分析**（layout analysis,识别页面是单栏/双栏/表格/图等区域） + **YOLO**（实时目标检测模型）检测留给 s11
- **表格丢失未修** — 572 字符仍在蒸发;s11 多模态抽取(`deepdoc/parser/docx_parser.py` 同时遍历 `paragraphs + tables`)是工业解法,本章不动
- **扫描件未修** — `extract_text()` 对图片型返回空,被静默过滤;s11 的 **VisionParser**（RAGFlow 内置的多模态文档解析器） + OCR(**PaddleOCR** 百度开源中文 OCR 引擎 / **Tesseract** Google 开源 OCR 引擎 / **GOT-OCR2** 端到端通用 OCR 模型)是兜底
...
- **PDF 多栏错位** — ``failure_modes.py`` 看到的 861/562/413/744 长度看着正常,但字符串内部语义混乱,embedding 会把碎段当正常段处理。s03 chunking 在 token-cap 级小块上重排,让多栏错位的影响在切片粒度被吸收;s11 版面分析(YOLO / **LayoutLMv3** 微软文档理解预训练模型)做根因修复
...
RAGFlow 的设计印证了这点：`VisionParser`（视觉解析器,接 OCR 模型读图片里的文字）在 `deepdoc/parser/utils.py` 里是**单独的 dispatcher**（分发器,根据页面类型选择对应 parser）——只有当 `RAGFlowPdfParser` 判定某页是扫描件时才会被调用，主路径不被 OCR 阻塞。
