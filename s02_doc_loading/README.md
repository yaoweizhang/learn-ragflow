...
### 2.1 `load_pdf` + `load_docx` 归一到 `{text, page, source}` schema
...
### 2.3 实测 schema 片段 + 跨格式对齐说明

把 `c01` 跑在仓库自带的 `samples/` 上，得到的 schema 真实片段长这样（用于对照"统一 schema 在两格式上是同一形状"）：
...
### 3.1 把 01 喂给真实样本暴露多栏错位 + 丢 572 字符
...
> `c01` 的"未做表格抽取"基线输出见上节 `### 跑一遍`；这一节专门展示 `c02` 量化损失（多栏错位 + 缺失 572 字符）。

4 是白皮书 PDF 解析出的非空页数（4 页都有内容）；27 是披露报告里的非空段落数。**3 张表格的内容不在这里**——这正是 `c02` 要揭示的问题。

### 3.4 不修只量化的 demo 性质 + 表格/扫描件留给 s11
...
`c01` 的输出（`{text, page, source}` schema）直接喂给 `c02` 做失败检测；`c01` 的输出同样直接喂给 s03 / s04 / s05 的整条离线管线。`c02` 不生产新数据，只暴露 `c01` 在 prod 上的损失量化——它的存在意义是"教学动机"，不参与主链路。
