...
>
> **章节结构**：2 个脚本。01 跑通 `pypdf` + `python-docx` 的最小骨架；02 把同一套函数喂给真实样本，把"看不见的损失"摆出来。
>
...
- **01**——跑通最小骨架（`pypdf` + `python-docx` → `list[{text, page, source}]`）；
- **02**——把同一套函数喂给真实样本，把"看不见的损失"摆出来：丢多少字、错位多严重、表里写了什么但读者看不见。
...
**如果你的真实语料里 80% 是扫描件 PDF，01 的链路对它们返回空字符串。要不要给 01 加一层 OCR 兜底？还是放到 s11 单独做？**

提示：放 01 会让所有用户都跑 OCR 模型，下载 1GB+；放 s11 是"按需启用"，更工程化。答案见下方"思考题答案"。
...
| `main()` (01) | `code_01_basic_load.py` | — | 打印段落数 + 片段 | 演示入口 |
| `show_pdf_failure()` | `code_02_failure_modes.py` | — | 打印每页长度与首字 | 把 01 的输出按页铺开，肉眼看出多栏错位 |
| `show_docx_table_loss()` | `code_02_failure_modes.py` | — | 打印表内字符数 | 直接量化"被吞掉的表格字数" |
| `main()` (02) | `code_02_failure_modes.py` | — | 调用上面两个 + 引出工业解法 | 02 演示入口 |
...
3. 给 02 加一段失败模式描述（"先暴露问题再换工具"的精神）。

不要在 `load_pdf` 里写 if-else 分发——它会污染单一职责。`load_pdf` 只懂 PDF，`main()` 懂全格式。

### 3.5 实际跑出来的 schema 形状

把 `01` 跑在仓库自带的 `samples/` 上，得到的真实片段长这样（用于对照"统一 schema 在两格式上是同一形状"）：
