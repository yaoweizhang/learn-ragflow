...
|---|---|---|---|
| [01_prompt_template](./units/01_prompt_template/README.md) | Prompt 模板 + LLM 引用生成(`<context>` 注入 + `[i]` 角标 + 拒答兜底 + graceful skip) | "s07 给候选,LLM 该怎么输出可追溯的答案" | `rag/prompts/generator.py` 的多 pass 拆解 + `citation_plus` 双 pass + `message_fit_in` fallback |
...
RAGFlow 在 `rag/prompts/` 包里维护 40+ 个独立 `.md` 模板(`generator.py` / `sufficiency_check.py` / `citation_prompt.py` / `multi_queries_gen.py` ...),把"MVP 的 `PROMPT` 一段话干的活"拆成 3-4 次独立 LLM 调用,核心三个:

- **`sufficiency_check`** — 判资料够不够,显式输出 `is_sufficient: true/false`;`False` 时直接走追问 / 拒答分支,不调 LLM 生成。**比 prompt 里写"没有就拒答"准一个数量级**——LLM 在独立判断时不会因为"还要生成答案"而被 prompt 后半段稀释。
- **`multi_queries_gen`** — 资料不够时改写 query(扩写、抽象、具体化),拿新 query 重跑检索,直到 `is_sufficient` 通过或重试上限。**这是 RAGFlow 处理"资料不全"的核心机制**——MVP 没有,生产必须接。
- **`citation_prompt` / `citation_plus`** — 先生成答案、再用 `citation_prompt` 让 LLM 二轮给句子级补 citation;`citation_plus` 是双 pass 升级版(答 + 标拆成两个 LLM 调用)。**引用准确率 +20-30%** 的代价是 token 成本 +100%。

完整摘录与 3 条"为什么这样写"的分析见 [`ragflow_notes/prompt_templates.md`](../ragflow_notes/prompt_templates.md)。**一句话对比**:RAGFlow 把"问答 prompt"做成**多 pass 流水线**——`sufficiency_check` (判够不够) → `multi_queries_gen` (不够就改写) → `citation_prompt` / `citation_plus` (双 pass 补引用)。**可观测、可分支、可替换,但贵**(2-3x token)。本章 MVP 走单段 PROMPT,生产建议至少升 sufficiency_check。
