...
### 1.1 核心定义：什么是 prompt 模板 + 上下文注入？三段式 + 定界符 + 角标
...
### 1.2 真实世界的问题：Prompt injection / Token overflow / Citation misalignment
...
## 二、prompt 模板 + LLM 引用生成：[c01_prompt_template.py](c01_prompt_template.py)
...
| 函数 | 文件 | 输入 | 输出 | 一句话解释 |
|---|---|---|---|---|
| `PROMPT` | `c01_prompt_template.py` | — | `str` | 4 条硬约束的 prompt 模板(只能依据 `<context>` / 没有就拒答 / 引用用 `[i]` 角标 / 中文+简洁) |
| `_format_context(hits)` | `c01_prompt_template.py` | `list[{text, source, page, ...}]` | `str` | 把 hits 渲染成 `[i] (source#page) text` 块,跟 prompt 里的 `[1][2]` 一一对应 |
| `answer(question, hits)` | `c01_prompt_template.py` | `(str, list[dict])` | `dict{text, citations}` | 调 OpenAI 兼容 LLM 生成答案,返回 `text`(剥掉 `<think>...</think>`)+ `citations`(命中的 source/page);无 `LLM_API_KEY` 时返回 `[skipped: ...]` |
| `main()` (code_01) | `c01_prompt_template.py` | — | 打印 top-3 + answer + citations | code_01 演示入口,self-contained(内联 chroma + s04 BGE embed + s06 hybrid + s07 rerank);默认 query `"内存"`(EOFError 时兜底) |
