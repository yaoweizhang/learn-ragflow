...
>
> 注: **Docker**(容器引擎,把进程和依赖打包成独立镜像,可移植地跑在任何 Linux 上;s00 已介绍);**Docker Compose**(Compose,基于 YAML 的多容器编排工具,用 `docker-compose.yml` 声明服务 / 端口 / 卷 / 环境变量);**Dockerfile**(镜像构建配方,逐行 `FROM / COPY / RUN / CMD` 描述镜像层);**uvicorn**(ASGI 服务器,FastAPI 的标准运行器,`uvicorn app:app --host 0.0.0.0 --port 8000` 启服务)。
...
>
> 注: **pydantic**(Python 数据验证库,用类型注解定义 schema,自动校验请求 / 响应 JSON 并报错);**OpenAPI**(REST API 描述规范,FastAPI 据此自动生成 `/docs` 交互式文档)。
...
**第四, 冷启动慢 + 没有任何观测**。`POST /qa` 第一次请求 Chroma **hnswlib**(HNSW 图的 C++ 实现,Chroma 用的 ANN 索引后端)要 **mmap**(memory map,把磁盘文件映射到进程虚拟内存,省去加载时间,但首次访问触发缺页中断)索引文件,3-5 秒延迟;之后没人知道"现在 /qa 平均延迟多少 / 错误率多少 / LLM 调用成本多少"。**生产解法**: ① 容器预热 (docker compose run 预热后 SIGTERM 再起服务); ② 加 `/healthz` 端点给 **K8s liveness probe**(Kubernetes 容器编排平台的存活探针,定期 HTTP GET `/healthz`,失败则重启 Pod); ③ 暴露 Prometheus `/metrics` 抓 `请求量 / 延迟分位 / 错误率 / LLM token 消耗`。MVP 都不带。
>
> 注: **K8s / Kubernetes**(容器编排平台,管理多副本 / 滚动升级 / 服务发现 / 自动扩缩容);**Prometheus**(时序数据库,定期 scrape 服务的 `/metrics` 端点存指标);**SIGTERM**(进程终止信号,docker stop 默认发 SIGTERM,容器应捕获并优雅退出)。
...
> 注: `QARequest(BaseModel)`(pydantic 模型,声明 HTTP 请求 JSON schema;`BaseModel` 继承后字段自动校验);**端口映射** `8000:8000`(把容器内 8000 端口暴露到主机 8000 端口);**volume 挂载** `samples:/app/samples:ro`(把主机目录挂进容器,`:ro` 表示容器内只读);`env_file: ../.env`(让 Docker 自动加载主机 .env 文件为容器环境变量);`subprocess.run([...], cwd=...)`(启动子进程并切到指定工作目录,`check=True` 让非零退出码抛异常)。
...
**docker-compose 声明式编排**:把"镜像怎么 build、端口怎么映射、卷怎么挂、环境怎么传"全部声明成 YAML,让编排成为可复现的配置文件而不是命令脚本。关键设计: ① `build.context` 是项目根 — Dockerfile 能 `COPY .. /app` 把整个仓库代码带进镜像,包括 s04-s11 的所有依赖;② `env_file: ../.env` — 容器内 `/app/.env` 自动被 Docker 加载,s08 进程读 `LLM_API_KEY` 跟本地一致;③ 只读挂载 (`:ro`,read-only) — 容器不会误写索引 / 样本,重 build 不丢数据;④ 路径用 `../` 前缀 — 相对 `docker-compose.yml` 文件位置解析,本地跑和 CI 跑行为一致。

| 文件 | 解决什么 | 留下什么局限 | 何时用 |
|---|---|---|---|
| `fastapi_docker.py` | 一行启动 + 两道前置校验 (.env / 索引) | 没 healthcheck / 没 rolling restart / 没预热 / 没鉴权 / 没限流 | 教学 / 本地起 demo / 单人开发 / 末章收尾 |
...
4. **优雅降级**(graceful degradation,环境不满足时退化为打印提示而不崩溃): `import shutil; if shutil.which("docker") is None` — 打印该敲的命令 (`cd s12_deployment && docker compose up --build`),CI / 学生笔记本无 Docker 时不崩
...
`app.py` / `Dockerfile` / `docker-compose.yml` 三个 supporting 文件的职责:**app.py** 是 FastAPI 应用,内部 `_get_col()` lazy-load Chroma collection,`qa(req)` handler 把 `_get_col → embed → hybrid_search → rerank → answer` 五步串行,索引缺失返 `HTTPException(503)`(HTTP 503 Service Unavailable,服务暂不可用);**Dockerfile** 是 `python:3.11-slim` + `tesseract-ocr` + `build-essential` + `pip install -r requirements.txt` + 业务代码,`CMD ["uvicorn", "s12_deployment.app:app", ...]` 启服务;**docker-compose.yml** 是 `services.rag: { build.context=.., dockerfile=s12_deployment/Dockerfile, ports=[8000:8000], env_file=[../.env], volumes=[../samples:/app/samples:ro, ../s05_vector_index/_chroma:/app/s05_vector_index/_chroma] }`。
...
``fastapi_docker.py`` + `app.py` + `Dockerfile` + `docker-compose.yml` 用 **单容器 FastAPI + compose 一键起服务** 解决了"让别人用 curl 就能调"的问题,但**留了 4 类典型生产缺口**:① **没 health-check**(`/healthz` 健康检查端点,K8s / compose 定期 HTTP GET 判断容器是否存活;`docker compose up` 不等服务 `Application startup complete` 就认为"成功",新用户分不清是镜像坏了还是代码坏了,生产应该加 `healthcheck: test: ["CMD", "curl", "-f", "http://localhost:8000/healthz"]` + `depends_on: condition: service_healthy`(`depends_on` 控制服务启动顺序,`condition: service_healthy` 让 compose 等待 healthcheck 通过后再起下游));② **没 rolling restart / log streaming** — `Ctrl-C` 直接杀容器,没有 `restart: unless-stopped`(容器退出时自动重启,除非手动 stop)也没有 `docker compose logs -f` 统一入口;③ **索引没预热** — Chroma 的 hnswlib 首次 query 要 mmap 索引文件,冷启动第一次 `/qa` 会有 3-5 秒延迟;④ **没鉴权 / 限流**(`rate limit`,速率限制,单位时间内允许的最大请求数,防止滥用) — `POST /qa` 谁都能调,LLM token 钱随便烧,也没有 rate limit。
...
- **没 rolling restart / log streaming**(rolling restart,滚动重启:逐个替换旧副本而非一次性全停) — `Ctrl-C` 直接杀容器,日志只在当前终端;生产应该加 `restart: unless-stopped` + `docker compose logs -f` 统一入口。
...
- **单容器单进程** — 崩了手动 `docker compose up` 重启;生产应该 `depends_on: condition: service_healthy` + `docker compose up --scale rag=3`(`--scale` 起多副本,这里 3 个相同容器并行接收流量) + nginx 负载均衡(把请求分发到多个后端),数据层换 ES / Milvus 这种支持分布式 + 主从复制的引擎。
...
- **Grafana** (`grafana/grafana` 镜像） — 可视化看板工具。Prometheus 是数据源，画"过去 1 小时 **P99 延迟**"(P99,99th Percentile,99% 请求都低于该延迟值;生产看尾延迟而不是平均值)""LLM 调用成功率"这种看板。
...
- 关键配置：**数据脱敏**(PII redaction,把 API key / 用户邮箱等敏感字段从上报 payload 中移除) (`.env` 里的 `LLM_API_KEY` 千万别上报），**采样率** （生产建议 10%，全量打爆配额），**告警 webhook** （新错误 5 分钟内没人 ack 就 @oncall）。
...
- **vLLM** (LLM 推理引擎) 或 **TEI** (Text Embeddings Inference,Embedding 推理) / **TGI** (Text Generation Inference,Rerank 推理) — 把模型独立成 HTTP 服务，gpu 显存占满后通过 batch + **PagedAttention**(vLLM 的显存分页管理,借鉴 OS 虚拟内存,把 KV cache 切成固定大小页减少显存碎片,撑高并发)撑高并发。API 进程只发 HTTP，几毫秒返回，可以同时跑几十个请求。
...
- **S3** (AWS / 阿里云 OSS / 腾讯云 COS，云上托管的对象存储) — 按量付费，11 个 9 持久性。
- **MinIO** — 自建对象存储，API 兼容 S3，放内网。最小两节点做副本，几个 TB 容量随便堆。
- 配套：上传走预签名 URL（presigned URL,带临时签名的对象访问链接,允许前端直传到 S3 而不暴露密钥） （前端直传，不经过 API），API 只存对象 key；处理流水线监听 S3 事件 (S3 Event Notification) 触发解析。
...
| 日志 | 容器 stdout | Loki / **ELK**(Elasticsearch + Logstash + Kibana,传统日志栈) |
| 错误追踪 | 无 | Sentry |
| 模型推理 | 进程内 | vLLM / TGI 独立服务 |
| 文件存储 | samples 目录 | S3 / MinIO |
| 鉴权 | 无 | API key / **JWT**(JSON Web Token,无状态身份令牌,服务端签名 + 客户端携带)/ OAuth2 |
| 队列 | 无 | **Celery** (Python 分布式任务队列) / **Redis Streams** (Redis 5+ 的消息流,轻量队列) |

MVP 是"能跑"，上面这些是"能上线"。中间还隔着一个"能卖钱"——那个阶段要做的事情更多 (**A/B 测试**,把用户分流到不同版本对比指标 / **灰度发布**,按百分比逐步放量新版本 / 多区域容灾），但和 RAG 本身没关系了，属于通用 SaaS 工程范畴。
...
3 个方向叠加：① 进程级——加 `restart: unless-stopped`(compose 字段，容器崩了自动拉起）+ 加 `/healthz` 端点给 K8s liveness probe（崩了不优雅退出会被 probe 杀掉）；② 流量级——compose scale 起来多副本（`docker compose up --scale rag=3`），前面套 nginx(反向代理 + 负载均衡) / **APISIX**(云原生 API 网关,支持鉴权 / 限流 / 灰度) 做负载均衡 + 健康检查剔除坏副本；③ 数据级——Chroma 索引是本地文件，**多副本同时读没问题，同时写会锁**。生产里换 ES / Milvus 这种原生支持分布式 + 主从复制的引擎，`depends_on: condition: service_healthy` 串启动顺序 + `replicas: 3`(`replicas` K8s Deployment 字段,期望运行的 Pod 副本数) 设副本数。MVP 不做高可用——单进程单容器，崩了手动 `docker compose up` 重启即可。
