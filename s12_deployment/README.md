...
s08 的 `answer()` 只能在命令行跑 `python s08_prompt_generate/c01_prompt_template.py`,然后交互式输入问题 — **s11 结束时的链路是"开发者本人在自己机器上跑得通"**。这条"本机能跑"和"别人能用"之间隔着四道典型鸿沟,每一道都对应一类工业级失败:
...
**第三, 重启 = 数据丢失 / 索引丢失**。本地跑 `python s08_prompt_generate/c01_prompt_template.py` 时索引在内存;kill 重启就重建。多人协作 / 服务器跑就尴尬。**生产解法**: 把 Chroma 索引目录用 volume 挂载进容器 (`:ro` 只读),容器重启不丢、重新 build 不丢、多副本读同一份索引。
...
```
   ...
   启动器 c01.py: .env gating + 索引 gating + subprocess docker compose up
```
...
s12 用 **4 个文件 + 1 个启动器** 把"s08 本机命令行 RAG"升级为"容器化 HTTP 服务"。`c01_fastapi_docker.py` 启动器先做两道前置校验 (`.env` 存在、`s05_vector_index/_chroma` 存在),然后 `subprocess.run(["docker", "compose", "up", "--build"], cwd=s12_deployment)` — 一次启动管全部,没有多档梯度。s12 只有这一步,因为整章就 4 个文件 (`app.py` / `Dockerfile` / `docker-compose.yml` + 本 `c01_fastapi_docker.py`),组合只有一种,`docker compose up --build` 是终点,没有中间状态可设。
...
| 文件 | 解决什么 | 留下什么局限 | 何时用 |
|---|---|---|---|
| `c01_fastapi_docker.py` | 一行启动 + 两道前置校验 (.env / 索引) | 没 healthcheck / 没 rolling restart / 没预热 / 没鉴权限流 | 教学 / 本地起 demo / 单人开发 / 末章收尾 |
...
## 代码 1: fastapi_docker 一行启动 ([c01_fastapi_docker.py](c01_fastapi_docker.py))

入口:[`c01_fastapi_docker.py`](c01_fastapi_docker.py)
...
**做一件事**: 跑 `python s12_deployment/c01_fastapi_docker.py`,内部先做 `.env` 存在 + 索引存在两道 gating,通过后 `subprocess.run(["docker", "compose", "up", "--build"], cwd=s12_deployment)` — 让 `app.py` (FastAPI 包装) + `Dockerfile` (镜像) + `docker-compose.yml` (编排) 三件套自动 build + 起来。

**N 步**:
1. 计算 `WORKDIR = Path(__file__).resolve().parents[1]` (项目根, 从 `s12_deployment/c01_fastapi_docker.py` 上溯 2 级) 和 `S12_DIR = WORKDIR / "s12_deployment"`
...
# 2. 启动 (任选一种)
python s12_deployment/c01_fastapi_docker.py
...
# 4. 停掉
# 在跑 c01_fastapi_docker.py 的终端 Ctrl-C
```
...
s12 把"s08 本机命令行 RAG"升级为"容器化 HTTP 服务" — `c01_fastapi_docker.py` 启动器 + `app.py` FastAPI 包装 + `Dockerfile` 镜像 + `docker-compose.yml` 编排 4 件套收敛成 `POST /qa` 一个端点。但每一步都留下生产缺口,这些是真实部署要继续填的洞:
...
1. **生产环境你还会加哪些服务？**
2. **为什么 `c01_fastapi_docker.py` 在 `.env` 不存在时直接退出、不进入 docker build？**
...
### Q2. 为什么 `c01_fastapi_docker.py` 在 `.env` 不存在时直接退出、不进入 docker build?
...
> Docker 缺命令 / compose 路径错 / `pip install` 编译错 / `/qa` 500 系列 / gating 报错 等排错细节见 `c01` 的 `### 局限与下一步`。