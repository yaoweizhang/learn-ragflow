...
```
[s00 概念速览] ──▶ [s01 最小链路] ──▶ [s02-s05 数据/索引] ──▶ [s06-s10 检索/生成/Agent] ──▶ [s11-s12 多模态/部署]
   (本章)        朴素 → 向量 → LLM     加载/切块/嵌入/向量库     混合/重排/Prompt/Agent/Graph     OCR + 部署
```

> s00 给下面 12 章提供概念地图。读完 s00 后，s01 的 "朴素 → 向量 → 检索 + LLM"、s06 的 "向量 + BM25 融合"、s09 的 "Agent 路由" 为什么这样设计，每一步都有锚点。
