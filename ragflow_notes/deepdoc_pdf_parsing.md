...
- **跟我们 30 行版本的差异**。我们的 `code.py` 只有 `PdfReader(path).pages[i].extract_text()` 一条路，对扫描件 / 表格 / 页眉页脚毫无办法。RAGFlow 把"读 PDF"拆成三个阶段：(1) `__images__` 把页面变成 72 × `zoomin` DPI 的图；(2) 视觉模型（OCR + 布局识别 + 表格结构识别）从图里读出文字与坐标；(3) 把结果按 bbox 排序回填成段落。代价是：要装 ONNX 模型 + GPU/CPU 推理 + 几十 MB 依赖，但换来"扫描件 / 复杂版式 / 表格"全场景能跑。这也是 s11 多模态会展开的方向。