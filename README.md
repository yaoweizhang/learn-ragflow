...
[English](./README.en.md) · [贡献指南](./docs/CONTRIBUTING.md) · [更新日志](./docs/CHANGELOG.md)
...
- **右线 —— 看源码**：每章 [`ragflow_notes/<topic>.md`](./ragflow_notes/) 摘录 RAGFlow 工业级实现的 5-15 行关键代码 + 行号 + commit pin + 为什么这样写的注释。
...
- 用 [`ragflow_notes/`](./ragflow_notes/) 给你**生产代码的真实设计**——RAGFlow 是 GitHub 上 star 数最高的开源 RAG 引擎之一、把上面每一环都工程化了、代码可读性高，正好做教材。
...
- 不需要先看过 RAGFlow 源码——摘录的部分足够让你跟读；
- 不需要深度学习/Transformer 背景——提到时会点到为止；
- 不需要 GPU——BGE embedding 在 CPU 上 1-2 分钟跑完。

## 项目亮点

1. **最小可运行 + 工业对照**：每章都是"先写 30 行的玩具，再看 RAGFlow 1000 行的设计"——既能跑，又知道生产真实长什么样。
2. **RAGFlow 源码摘录**：每章配一张源码摘录卡，pin 在 commit `828c5789f`（2026-07-01），含行号、commit hash、为什么这样写——可直接跳到 RAGFlow 仓库对应行号读。
3. **共享样本文件**：仓库随附两个虚构样本（`server_whitepaper.pdf`、`disclosure.docx`），跨章复用，方便对照各章输出。
4. **环境变量 + 单一依赖清单**：12 章共用一份 `requirements.txt`、一份 `.env.example`，跑通不踩坑。
5. **思考题 + 答案分离**：每章末尾的"思考题"在 `thinking_answers.md` 单独放，鼓励先自己想再看。
...
- [ ] *（延伸阅读）* [RAG 技术综述](./docs/00_introduction/01_what_is_rag.md)（参数化 vs. 非参数化知识；初级 / 高级 / 模块化 RAG）
...
- [x] RAGFlow 多服务编排（api / web / mysql / es / minio / redis）对照

### 补充阅读（项目级，不参与教程运行）

- [RAG 技术综述](./docs/00_introduction/01_what_is_rag.md) — RAG 是什么、与长上下文的权衡、技术演进
- [项目级 README 补充](./docs/00_introduction/README.md) — `docs/` 目录的使用方法 + 与 `sXX_topic/` 的边界
- [RAGFlow 源码摘录索引](./ragflow_notes/README.md) — pin 版本 + 免责说明
...
python s01_what_is_rag/code.py    # 第 1 章：30 行就够
```
...
├── docs/                         # ← 项目级文档（只读，补充阅读）
│   └── 00_introduction/
│       ├── README.md             # docs/ 的边界与用法
│       └── 01_what_is_rag.md     # RAG 技术综述（17KB，all-in-rag 风格）
├── ragflow_notes/                # ← 每章对照 RAGFlow 源码摘录（pin 在 828c5789f）
│   ├── README.md                 # 版本 pin + 免责说明
│   ├── deepdoc_pdf_parsing.md    # s02 对照
│   ├── deepdoc_chunking.md       # s03 对照
│   ├── embedding_routing.md      # s04 对照
│   ├── vector_indexing.md        # s05 对照
│   ├── hybrid_retrieval.md       # s06 对照
│   ├── rerank.md                 # s07 对照
│   ├── prompt_templates.md       # s08 对照
│   ├── agent_tools.md            # s09 对照
│   ├── graph_extraction.md       # s10 对照
│   ├── multimodal_parsing.md     # s11 对照
│   └── deployment.md             # s12 对照
└── s01_what_is_rag/              # 章节 1（README + code.py + README.en + 思考题答案）
    ...
...
- **接 RAGFlow 源码**：每章 `ragflow_notes/<topic>.md` 摘了 RAGFlow 关键 5-15 行；从 s01 的 `rag/nlp/search.py` 一路读到 s12 的 `docker/docker-compose.yml`。
...
- [**learn-claude-code**](https://github.com/shareAI-lab/learn-claude-code) — 本教程的"最小可运行 MVP + 工业对照"风格借鉴此仓库；每章 README 的"问题 / 最小解法 / 跑起来 / 思考题"结构也来自此。
