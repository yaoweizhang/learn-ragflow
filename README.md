...
- **正文章节**：每章 1 个 `README.md` + 同级目录里 N 个 `<topic>.py`（描述性命名，无 `c0N_`/`code_NN_` 前缀），30-80 行可单独运行；你可以改一行、看输出变化，把"调大 alpha 是不是召回更好"这类问题在 5 分钟内验证。
- **工业参考（默认阅读路径，唯一工业对照）**：[RAGFlow](https://github.com/infiniflow/ragflow) 是 GitHub 上 star 数最高的开源 RAG 引擎之一，把 RAG 全链路的每一环都做了工程化、可独立替换的模块。本教程把 RAGFlow 当作**唯一**的工业参考——每章末的 `## RAGFlow 实现` 小节摘录 RAGFlow 在该层的工程化做法；详细源码分析见 [`docs/reference/ragflow-notes/`](./docs/reference/ragflow-notes/)（按章集中放仓库顶层：5-15 行关键代码 + 行号 + 为什么这样写的注释，做深读时再翻，不在每章正文里复读）。**ragflow-notes 不是补充材料，是教学路径的另一半**。
...
教程分 **5 部分、12 章**,每章一个 `README.md` + 同级 `<topic>.py`(描述性命名):
...
- [RAGFlow 源码阅读索引](./docs/reference/ragflow-notes/README.md) — 工业级实现（想做深读时翻）
...
│   └── reference/
│       └── ragflow-notes/        # RAGFlow 工程化做法（按章对应）
│           ├── README.md         # 摘录索引 + 阅读建议
...
│   └── README.md
├── s01_what_is_rag/              # ← 章节 1：1 个 README + 3 个 <topic>.py
│   ├── README.md
│   ├── substring_match.py
│   ├── bag_of_words.py
│   └── rag_pipeline.py
...
- 每章 30-60 分钟：跑 `<topic>.py` + 改 `<topic>.py` 看变化 + 读对应 `docs/reference/ragflow-notes/<topic>.md`。
...
- **接 RAGFlow 源码**：在 [`docs/reference/ragflow-notes/`](./docs/reference/ragflow-notes/) 里按章对应了 RAGFlow 工程化做法；重点学**设计意图**，不是逐行背诵。

## 引用与致谢

本教程的工业级对照来自以下开源项目：

- [**RAGFlow**](https://github.com/infiniflow/ragflow) — 主力参考实现。见 [`docs/reference/ragflow-notes/`](./docs/reference/ragflow-notes/)。RAGFlow 持续演化，与最新版可能不一致。
