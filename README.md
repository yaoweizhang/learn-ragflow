...
- **左线 —— 动手做**：每章 1 个 `README.md` + 同级目录里 N 个 `code_NN_<topic>.py`(描述性命名,不再用 `01_xxx/02_xxx/` 子目录),30-80 行可单独运行;你可以改一行、看输出变化,把"调大 alpha 是不是召回更好"这类问题在 5 分钟内验证。
- **右线 —— 看源码**:每章 [`docs/reference/ragflow-notes/<topic>.md`](./docs/reference/ragflow-notes/) 摘录 RAGFlow 工业级实现的 5-15 行关键代码 + 行号 + 为什么这样写的注释。
